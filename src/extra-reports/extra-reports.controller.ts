...
export class ExtraReportsController {
  ...
  @Get('custom')
  async getCustomReports(@Res() response: Response) {
    const pdfDoc = this.extraReportsService.getCustom();

    response.setHeader('Content-Type', 'application/pdf');
    pdfDoc.info.Title = 'Billing.pdf';
    pdfDoc.pipe(response);
    pdfDoc.end();
  }

  @Get('custom-size')
  async getCustomSizeReports(@Res() response: Response) {
